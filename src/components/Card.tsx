  ...
  if (isBack) {
    return (
      <motion.div
        ...
        className={`w-16 h-24 sm:w-24 sm:h-36 bg-emerald-700 rounded-lg border-2 border-white/20 shadow-lg flex items-center justify-center relative overflow-hidden ${className}`}
      >
        <div className="absolute inset-2 border border-white/10 rounded-md flex items-center justify-center">
          <div className="w-8 h-8 sm:w-12 sm:h-12 border-2 border-white/20 rounded-full rotate-45 flex items-center justify-center">
  ...
  return (
    ...
    >
      <div className={`flex flex-col items-start ${colorClass}`}>
        ...
        <span className="text-xs sm:text-base leading-none">{symbol}</span>
      </div>
      
      <div className={`absolute inset-0 flex items-center justify-center text-2xl sm:text-4xl ${colorClass} opacity-20`}>
        {symbol}
      </div>
      ...
      {card.avatarUrl && (
        <div className="absolute inset-0 flex items-center justify-center p-4 pointer-events-none">
          <img 
            src={card.avatarUrl} 
            alt="Eggy" 
            className="w-12 h-12 sm:w-16 sm:h-16 rounded-full object-cover border-2 border-slate-100 shadow-inner"
